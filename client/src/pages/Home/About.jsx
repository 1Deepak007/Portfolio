...
const About = () => {
  ...
  const {lottieURL,description1,description2,skills,skillsImage} = abouts
            ...
            <div className="grid grid-cols-2 gap-4 mt-5 border-blue-400 sm:grid-cols-4">
              {skills.map((skill, index) => (
                <div key={index} className="flex items-center mt-5 text-white">
                  {/* <img src={skillsImage[index]} alt={skill} className="w-6 h-6 mr-4 bg-slate-900 rounded-md md:w-10 md:h-10" /> */}
                  {/* {
                    skillsImage.map((image)=>{
                      <img src={image[index]} alt={skill} className="w-6 h-6 mr-4 bg-slate-900 rounded-md md:w-10 md:h-10" />
                    })
                  } */}
